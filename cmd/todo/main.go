...
import (
	...
	"bitbucket.innouk.music-group.com/Learning/protobufs/proto1/todo"
...
func add(text string) error {
	...
	if err != nil {
		return fmt.Errorf("could not encode error: %v", err)
	}
...
func list() error {
	...
	if err != nil {
		return fmt.Errorf("could not read from file %s: %v", dbPath, err)
	}
	...
	for {
		if len(b) == 0 {
			...
			return fmt.Errorf("remaining bytes %d for what???", len(b))
		}
